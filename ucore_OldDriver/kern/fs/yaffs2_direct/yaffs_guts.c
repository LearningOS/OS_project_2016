...
static void yaffs_remove_obj_from_dir(struct yaffs_obj *obj)
{
	...
	yaffs_yaffs_list_del_init(&obj->siblings);
	obj->parent = NULL;
...
void yaffs_add_obj_to_dir(struct yaffs_obj *directory, struct yaffs_obj *obj)
{
	...
	/* Now add it */
	yaffs_list_add(&obj->siblings, &directory->variant.dir_variant.children);
...
static void yaffs_unhash_obj(struct yaffs_obj *obj)
{
	...
	if (!yaffs_list_empty(&obj->hash_link)) {
		yaffs_yaffs_list_del_init(&obj->hash_link);
...
static void yaffs_free_obj(struct yaffs_obj *obj)
{
	...
	if (obj->parent)
		BUG();
	if (!yaffs_list_empty(&obj->siblings))
		BUG();
...
static void yaffs_hash_obj(struct yaffs_obj *in)
{
	...
	yaffs_list_add(&in->hash_link, &dev->obj_bucket[bucket].list);
	dev->obj_bucket[bucket].count++;
}
...
void yaffs_update_dirty_dirs(struct yaffs_dev *dev)
{
	...
	while (!yaffs_list_empty(&dev->dirty_dirs)) {
		link = dev->dirty_dirs.next;
		yaffs_yaffs_list_del_init(link);
					  ...
					  const YCHAR *alias_str, u32 rdev)
{
	...
	case YAFFS_OBJECT_TYPE_HARDLINK:
		...
		yaffs_list_add(&in->hard_links, &equiv_obj->hard_links);
		break;
	case YAFFS_OBJECT_TYPE_FILE:
	case YAFFS_OBJECT_TYPE_DIRECTORY:
	case YAFFS_OBJECT_TYPE_SPECIAL:
...
int yaffs_is_non_empty_dir(struct yaffs_obj *obj)
{
	return (obj &&
		...
		!(yaffs_list_empty(&obj->variant.dir_variant.children));
}
...
static int yaffs_del_link(struct yaffs_obj *in)
{
	 ...
	 * object
	 */
	yaffs_yaffs_list_del_init(&in->hard_links);
	return yaffs_generic_obj_del(in);
}
...
int yaffs_del_obj(struct yaffs_obj *obj)
{
	...
	case YAFFS_OBJECT_TYPE_FILE:
		...
		break;
	case YAFFS_OBJECT_TYPE_DIRECTORY:
		if (!yaffs_list_empty(&obj->variant.dir_variant.dirty)) {
			yaffs_trace(YAFFS_TRACE_BACKGROUND,
				"Remove object %d from dirty directories",
				obj->obj_id);
			yaffs_yaffs_list_del_init(&obj->variant.dir_variant.dirty);
		}
...
static int yaffs_unlink_worker(struct yaffs_obj *obj)
{
	...
	if (obj->variant_type == YAFFS_OBJECT_TYPE_HARDLINK) {
	...
	} else if (!yaffs_list_empty(&obj->hard_links)) {
		...
		yaffs_yaffs_list_del_init(&hl->hard_links);
...
void yaffs_link_fixup(struct yaffs_dev *dev, struct list_head *hard_list)
{
	...
	list_for_each_safe(lh, save, hard_list) {
		...
		if (in) {
			...
			yaffs_list_add(&hl->hard_links, &in->hard_links);
		} else {
