...
LIST_HEAD(yaffsfs_deviceList); // a empty bi list named yaffsfs_deviceList
...
int yaffs_mount_common(const YCHAR *path,int read_only, int skip_checkpt, struct yaffs_dev** dev_stored)
{
	int retVal=-1;
	int result=YAFFS_FAIL;
	struct yaffs_dev *dev=NULL;
...
{
	return yaffs_mount_common(path, readonly, 0,0);
}
int yaffs_mount(const YCHAR *path)
{
	return yaffs_mount_common(path, 0, 0, 0);
}
...
void yaffs_add_device(struct yaffs_dev *dev)
{
	...
	yaffs_list_add(&dev->dev_list,&yaffsfs_deviceList);
}

void yaffs_remove_device(struct yaffs_dev *dev)
{
	yaffs_yaffs_list_del_init(&dev->dev_list);
}
...
static void yaffsfs_SetDirRewound(yaffsfs_DirectorySearchContext *dsc)
{
	if(dsc &&
	   ...
	   dsc->dirObj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY){
           ...
           if( yaffs_list_empty(&dsc->dirObj->variant.dir_variant.children))
                dsc->nextReturn = NULL;
           else
...
struct yaffs_obj* yaffs_helper_get_nth_direntry(struct yaffs_obj* dir, int n)
{
  ...
  struct yaffs_obj *obj = list_entry(ptr,struct yaffs_obj,siblings);
  return obj;
}
	...
	if(!obj && notDir)
		...
		if(dsc){
			memset(dsc,0,sizeof(yaffsfs_DirectorySearchContext));
                        ...
                        yaffs_list_add(&dsc->others,&search_contexts);
...
int yaffs_closedir(yaffs_DIR *dirp)
{
	...
	if(!dsc){
        ...
        yaffsfs_Lock();
        dsc->magic = 0;
        yaffs_list_del(&dsc->others); /* unhook from list */
        kfree(dsc);
        yaffsfs_Unlock();
        return 0;
}
...
void dump_directory_tree_worker(const char *dname,int recursive)
{
	...
	else
	{
		...
		{
			snprintf(str,500,"%s/%s",dname,de->d_name);

			yaffs_lstat(str,&s);

			kprintf("%s inode %d obj %x length %d mode %x ",
				str,s.st_ino,de->d_dont_use, s.st_size,s.st_mode);
			switch(s.st_mode & S_IFMT)
			{
				case S_IFREG: kprintf("data file"); break;
				case S_IFDIR: kprintf("directory"); break;
				case S_IFLNK: kprintf("symlink -->");
...
void yaffs_dump_dir(const char *dname)
{
