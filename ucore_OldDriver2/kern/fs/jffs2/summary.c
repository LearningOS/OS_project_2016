...
{
	if (!s->sum_list_entry)
		s->sum_list_entry = (union jffs2_sum_mem *) item;
	if (s->sum_list_tail)
...
static void jffs2_sum_clean_collected(struct jffs2_summary *s)
{
	...
	while (s->sum_list_entry) {
		temp = s->sum_list_entry;
		s->sum_list_entry = s->sum_list_entry->u.next;
		kfree(temp);
	}
...
void jffs2_sum_move_collected(struct jffs2_sb_info *c, struct jffs2_summary *s)
{
	...
	c->summary->sum_list_entry = s->sum_list_entry;
	c->summary->sum_list_tail = s->sum_list_tail;

	s->sum_list_entry = s->sum_list_tail = NULL;
}
				...
				uint32_t infosize, uint32_t datasize, int padsize)
{
	...
	while (c->summary->sum_num) {
		...
		c->summary->sum_list_entry = temp->u.next;
		kfree(temp);
	...
	__must_hold(&c->erase_completion_block)
{
	...
	if (!c->summary->sum_num || !c->summary->sum_list_entry) {
