...
static int add_data_watchpoint(naive_mips_t *sl, enum watchfun wf, mips_addr_t addr)
{
    int i;
...
static int del_data_watchpoint(naive_mips_t *sl, enum watchfun wf, mips_addr_t addr)
{
    int i;
    for (i = 0 ; i < DATA_WATCH_NUM; i++)
    if ((data_watches[i].addr == addr) && (data_watches[i].fun == wf)) {
        data_watches[i].fun = WATCHDISABLED;
        sl->disable_data_wp(i);
        return 0;
    }
...
void *wait_for_cc(void* lpParameter){
    ELOG("Xcc\n");
    for(;;){
...
void *wait_for_cpu(void* lpParameter){
    for(;!NaiveMIPS.is_stop(););
    pthread_mutex_lock(c_mut);
    cont_stop=1;
    pthread_mutex_unlock(c_mut);
}
    ...
    while (1) {
        ...
        case 'Z': {
            ...
            case '4': { // insert access watchpoint
                ...
                if (add_data_watchpoint(sl, wf, addr) < 0) {
                    reply = strdup("E00");
                } else {
                    reply = strdup("OK");
                    break;
        ...
        case 'z': {
            ...
            case '4' : {// remove access watchpoint
                ...
                if (del_data_watchpoint(sl, wf, addr) < 0) {
                    reply = strdup("E00");
                } else {
                    reply = strdup("OK");
                    break;
