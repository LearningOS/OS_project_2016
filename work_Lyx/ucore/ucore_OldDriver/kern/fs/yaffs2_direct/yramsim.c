...
static SimData *yramsim_alloc_sim_data(u32 devId, u32 nBlocks)
{
	...
	sim = kmalloc(sizeof (SimData),0);
	if(!sim)
		return NULL;

	simDevs[devId] = sim;

	blockList = kmalloc(nBlocks * sizeof(Block *), 0);
...
static struct  yaffs_dev yramsim_dev0;
static char    yramsim_name0[64];
				...
				u32 start_block, u32 end_block)
{
	...
	g = kmalloc(sizeof(ynandif_Geometry), 0);
  strncpy(yramsim_name0, name, 62);
