...
#elif defined MACH_FPGA
    //TODO
    int com_state = (inw(COM1 + 0x04) & 0x02);
    int keyboard_state = (inw(KEYBOARD + 0x04) & 0x01);
    if(com_state == 0 && keyboard_state == 0)
      return -1;

    if (com_state){
        c = (inw(COM1 + 0x00) & 0xFF);
    }else{
        int tmp = inw(KEYBOARD + 0x00);
        c = (tmp < 0 || tmp > 256)?0:KEYCODE_MAP[tmp];
    }
