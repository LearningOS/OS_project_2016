...
//			expr.val = tr.genDiv(expr.left.val, expr.right.val);
			Label div_go = Label.createLabel();
			tr.genBnez(expr.right.val, div_go);
//			TODO: fill the zero check
//			String msg_div = RuntimeError.DIVIDE_ZERO;
			
			tr.genMark(div_go);
			tr.genParm(expr.left.val);
			tr.genParm(expr.right.val);
			expr.val = tr.genIntrinsicCall(Intrinsic.DIV);
			break;
		case Tree.MOD:
//			expr.val = tr.genMod(expr.left.val, expr.right.val);
			Label mod_go = Label.createLabel();
			tr.genBnez(expr.right.val, mod_go);
//			TODO: fill the zero check
//			String msg_div = RuntimeError.DIVIDE_ZERO;
			...
			tr.genParm(expr.left.val);
			tr.genParm(expr.right.val);
			expr.val = tr.genIntrinsicCall(Intrinsic.MOD);
			break;
		case Tree.AND:
