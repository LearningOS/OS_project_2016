...
//				break;
			...
			case GTR:
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT3, "slt", tac.op0.reg,
						tac.op2.reg, tac.op1.reg));
				break;
			case GEQ:
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT3, "slt", tac.op0.reg,
						tac.op1.reg, tac.op2.reg));
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT3, "xori", tac.op0.reg,
						tac.op0.reg, 1));
				break;
			case EQU:
...
//						tac.op1.reg, tac.op2.reg));
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT3, "subu", tac.op0.reg,
						tac.op1.reg, tac.op2.reg));
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT3, "sltu", tac.op0.reg,
						"$zero", tac.op0.reg));
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT3, "xori", tac.op0.reg,
						tac.op0.reg, 1));
				break;
...
//						tac.op1.reg, tac.op2.reg));
			...
			case NEG:
				...
				break;
			case LNOT:
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT3, "nor", tac.op0.reg,
						tac.op1.reg, "$zero"));
				break;
			case ASSIGN:
			...
			case LOAD_IMM4:
				...
				if (high == 0) {
					...
					bb.appendAsm(new MipsAsm(MipsAsm.FORMAT2, "lui",
							tac.op0.reg, high));
					if (low != 0) {
						bb.appendAsm(new MipsAsm(MipsAsm.FORMAT3, "ori",
	...
	private void genAsmForCall(BasicBlock bb, Tac call) {
		...
		if (call.opc == Tac.Kind.DIRECT_CALL) {
			...
			bb.appendAsm(new MipsAsm(MipsAsm.FORMAT0, "nop"));
		} else {
	...
	private void emitTrace(BasicBlock bb, FlowGraph graph) {
		...
		case BY_BRANCH:
			...
			if (directNext.mark) {
				...
				emit(null, String.format(MipsAsm.FORMAT0, "nop"), null);
			} else {
		...
		case BY_RETURN:
			if (bb.var != null) {
				...
				emit(null, String.format(MipsAsm.FORMAT2, "li", "$v0",
						0), null);
			}
